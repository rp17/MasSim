...
public class RavenGame {
	...
	public RavenGame() {
		...
		try {
			...
			//initBots();
			paused = Boolean.parseBoolean(RavenScript.getString("StartPaused"));
		} catch (IOException e) {
	...
	private void initBots() {
		...
		addNetNode(mobNode);
		INetNode node1 = new StaticNetNode("Static 1", this, new Vector2D(500.0, 195.0), 80.0);
		addNetNode(node1);
		INetNode node2 = new StaticNetNode("Static 2", this, new Vector2D(450.0, 295.0), 180.0);
		addNetNode(node2);
		INetNode node3 = new StaticNetNode("Static 3", this, new Vector2D(455.0, 255.0), 180.0);
		addNetNode(node3);
		mobNode.addNeighbor(node1);
		mobNode.addNeighbor(node2);
